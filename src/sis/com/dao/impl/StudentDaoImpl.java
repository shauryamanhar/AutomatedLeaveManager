...
public class StudentDaoImpl implements StudentDao {
	...
	*/	
	 ...
	 */
	@Override
	public Student ShowStudentById(long enrollmentNumber) {
		Student student=new Student();
		Connection con=null;
		ResultSet rs=null;
		 PreparedStatement pstmt=null;
	...
	pstmt=con.prepareStatement(sql);
	pstmt.setLong(1,enrollmentNumber);
	System.out.println("=============enrollment"+enrollmentNumber);
		rs=pstmt.executeQuery();
		 if(rs.next()){
			 System.out.println("getting student by id"+rs.getLong("enrollment"));
			 student.setEnrollmentNumber(rs.getLong("enroll"));
...
//shaurya=============================================
	...
	*/
	...
	public boolean insertLeave(Leave l) {
		try {
			con=SisDbUtil.getConnection();
			String sql="insert into leave_request(enroll,fromDate,toDate,subject,reason,entryDate)values(?,?,?,?,?,?)  ";
			PreparedStatement p = con.prepareStatement(sql);
			p.setLong(1, l.getEnroll());
			p.setDate(2, new java.sql.Date(l.getFromDate().getTime()));
			p.setDate(3, new java.sql.Date(l.getToDate().getTime()));
	...
	public List<Leave> approved() {
		...
		try {
			...
			while(rs.next()) {
				leave=DaoFactory.getLeave();
				leave.setLeaveId(rs.getLong("leaveId"));
				leave.setEnroll(rs.getLong("enroll"));
				leave.setFromDate(rs.getDate("fromDate"));
				leave.setToDate(rs.getDate("toDate"));
