...
public class AdminDaoImplement implements AdminDao {
	...
	public boolean validateLogin(String userName, String userPassword) {
		...
		try {
			...
			if (rs.next()) {
				...
				int result = pstmt.executeUpdate();
				/*
				 * if(result==1) System.out.println("lastLogin time insertion successful"); else
	...
	public boolean send(ComplainQuery c) {
		try {
			con = SisDbUtil.getConnection();
			String sql = "insert into query(queryId,sourceId," + "destinationId,message," + "subject,time)"
					+ "values(?,?," + "?,?," + "?,?)";
